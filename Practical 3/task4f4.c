...
//	    }
	  ...
	  for (int i = 0; i < NUM_SIZES; i++) {
	      int w = widths[i];
	      int h = heights[i];
	      int chunk_height = 800;  // process 100 rows at a time
...
//	      start_ms = HAL_GetTick();
//	      uint64_t cs_double = calculate_mandelbrot_double(w, h, MAX_ITER);
//	      end_ms = HAL_GetTick();
//	      exec_time_double_ms[i] = end_ms - start_ms;
//	      checksum_double[i] = cs_double;
//
//	      HAL_Delay(50);
	  }
